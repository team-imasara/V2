...
namespace GFHelper
{
    class ServerHelper
    {
        private static readonly string DefaultUserAgent = "Dalvik/2.1.0 (Linux; U; Android 6.0.1; HUAWEI RIO-AL00 Build/HuaweiRIO-AL00)";
        ...
        public static HttpWebResponse CreatePostHttpResponse(string url, IDictionary<string, string> parameters, Encoding charset)//2017.2.9添加byGK
        {
        ...
        //慎用
        public string SendDataToServer(string API, string data, bool ifProcessData = false)
        {
            ...
            string result = DoPost(Models.SimpleInfo.GameAdd + API, requeststring);
            ...
            if (ifProcessData)
            {
                ...
                im.listener.processMainData(API.ToString(),par,nresult);
            }
            return nresult;
        }
        ...
        public bool UploadBuildResult(dynamic clientjson, int gunid)
        {
            ...
            sb.Append(string.Format("&extra={0},{1},{2}", Models.SimpleInfo.uid, im.data.userInfo.level, Models.SimpleInfo.platform.ToString()));
